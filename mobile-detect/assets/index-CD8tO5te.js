...
var best_index : i32 = 0;`,`if (${n.getByIndices("input_indices")} ${t.selectLastIndex>0?">=":">"} value) {
  ...
  }`}},kw=e=>{Rm(e.inputs),e.compute(Nm(e.inputs))}}),Mm,ve,Sw,Cw,Ew,Tw,Iw,zw,Aw,Ow,Rw,Dm,Nw,Mw,Dw,Pw,Yi,Bw,Rs,Lw,Uw,jw,Ww,Fw,Vw,qw,Hw,Gw,Kw,Qw,Yw,Zw,Xw,Jw,e1,Kl,t1,bd,kd,r1,n1,i1,Pm,Bm,a1,Wc=V(()=>{se(),de(),Ue(),fe(),Mm=(e,t,r,n,a,i,s)=>{let o=Math.ceil(t/4),u="";typeof a=="string"?u=`${a}(a)`:u=a("a");let d=B("inputData",r,[o],4),f=te("outputData",n,[o],4),p=[{name:"vec_size",type:"u32"}];return s&&p.push(...s),`
...
`,Bm=e=>`quick_gelu_impl(${e})`,a1=(e,t)=>{let r=st(e.inputs[0].dataType);e.compute(ve(e.inputs[0],"QuickGelu",Bm,Pm(r,t.alpha),t.cacheKey,e.inputs[0].dataType))}}),Lm,Um,s1,Nk=V(()=>{de(),fe(),Wc(),Lm=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},Um=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let r=B("input",e[0].dataType,e[0].dims,4),n=B("bias",e[0].dataType,[e[0].dims[2]],4),a=te("output",e[0].dataType,t,4),i=M.size(t)/4,s=Ye(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(i/64)}}),getShaderSource:o=>`
...
`}return oe};return`
  ...
  `};return{name:"MatMulNaive",shaderCache:{hint:`${t.activation};${p};${m};${_};${a}`,inputDependencies:$?["rank","rank","rank"]:["rank","rank"]},getRunData:()=>({outputs:[{dims:i?i(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(w/64)},programUniforms:g}),getShaderSource:x}}}),Gm,Km,Sd,Ql,Qm,Cd,Ym,uo,Gc=V(()=>{se(),de(),fe(),Pn(),Hc(),Vc(),Gm=(e,t)=>e?`
...
`)};
    ...
    ${w}`},f=[{type:12,data:u},...ie(t,n)];return{name:"Expand",shaderCache:{hint:`${n.length};${s}${o}`,inputDependencies:["rank"]},getShaderSource:d,getRunData:()=>({outputs:[{dims:n,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(u/64)},programUniforms:f})}},M1=e=>{vg(e.inputs),e.compute($g(e.inputs),{inputs:[0]})}}),xg,D1,Kk=V(()=>{se(),de(),fe(),Wc(),xg=e=>{let t=e[0].dataType,r=M.size(e[0].dims),n=M.size(e[1].dims),a=n%4===0,i=s=>{let o=B("x",t,[1],4),u=B("bias",t,[1],4),d=te("y",t,[1],4),f=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],p=_=>`
  ...
  ${m.mainStart()}
    ...
    }`};return{name:"GatherBlockQuantized",shaderCache:{hint:`${t.cacheKey};${e.filter((_,w)=>w!==1).map(_=>_.dims.join("_")).join(";")}`,inputDependencies:Array.from({length:e.length},(_,w)=>"rank")},getRunData:()=>({outputs:[{dims:o,dataType:d}],dispatchGroup:{x:Math.ceil(u/64)},programUniforms:p}),getShaderSource:m}},j1=(e,t)=>{let r=e.inputs;Cg(r,t),e.compute(Eg(e.inputs,t))},W1=e=>xe({blockSize:e.blockSize,gatherAxis:e.gatherAxis,quantizeAxis:e.quantizeAxis})}),Tg,Ig,F1,V1,Xk=V(()=>{se(),de(),Ue(),fe(),Tg=e=>{if(!e||e.length!==2)throw new Error("GatherElements requires 2 inputs.");if(e[0].dims.length<1)throw new Error("GatherElements requires that the data input be rank >= 1.");if(e[0].dims.length!==e[1].dims.length)throw new Error(`GatherElements requires that the data input and
...
`)}
  ...
  ${C.mainStart()}
    ...
    ${g?"inv_std_output[global_idx] = inv_std_dev":""};
        ...
        }`};return{name:"BlockwiseMatMulNBits32",shaderCache:{hint:`${t.blockSize};${p};${m};${S};${$}`,inputDependencies:Array(e.length).fill("rank")},getRunData:()=>({outputs:[{dims:_,dataType:f}],dispatchGroup:{x:k},programUniforms:C}),getShaderSource:O}},o$=(e,t)=>{ty(e.inputs,t),t.blockSize===32&&e.adapterInfo.isVendor("intel")&&e.adapterInfo.isArchitecture("gen-12lp")?e.compute(ny(e.inputs,t)):e.compute(ry(e.inputs,t))},l$=e=>xe(e)}),iy,ay,sy,oy,ly,uy,dy,cy,u$,sS=V(()=>{se(),de(),fe(),iy=e=>{if(!e||e.length<1)throw new Error("Too few inputs");if(e[0].dataType!==1&&e[0].dataType!==10)throw new Error("Input type must be float or float16.");if(e.length>=2){let t=e[0].dims.length*2===e[1].dims[0];if(e.length===4&&(t=e[3].dims[0]*2===e[1].dims[0]),!t)throw new Error("The pads should be a 1D tensor of shape [2 * input_rank] or [2 * num_axes].")}},ay=(e,t,r)=>{let n="";for(let a=t-1;a>=0;--a)n+=`
  ...
  for (var i = 0u; i < uniforms.num_updates_elements; i++) {
    ...
    ${_y(t.reduction,"output[data_offset + i]","value",w.type.value)}
      ...
      }`};return{name:"ScatterND",shaderCache:{hint:`${t.cacheKey}_${t.reduction}`,inputDependencies:["rank","rank"]},getRunData:()=>({outputs:[{dims:a,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)},programUniforms:d}),getShaderSource:f}},$$=e=>xe({reduction:e.reduction}),x$=(e,t)=>{e.compute(vy(e.inputs,t),{inputs:[e.inputs[1],e.inputs[2]],outputs:[]})}}),wy,$y,xy,mu,by,ky,Sy,Cy,Ey,Ty,Iy,zy,gu,Ay,Oy,Ry,Ny,My,b$,k$,cS=V(()=>{se(),de(),Ue(),fe(),wy=(e,t)=>{if(e.every(r=>r>0||(()=>{throw new Error("Resize requires scales input values to be positive")})),e.length>0){if(t.mode==="linear"){if(!(e.length===2||e.length===3||e.length===4&&e[0]===1&&e[1]===1||e.length===4&&e[0]===1&&e[3]===1||e.length===5&&e[0]===1&&e[1]===1))throw new Error(`For linear mode, Resize requires scales to be 2D, 3D, 4D with either two outermost or one innermost and
